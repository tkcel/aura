...
export class LLMService {
  ...
  public async processTextWithImage(text: string, imageUrl: string, agent: Agent): Promise<LLMResult> {
    if (!this.openai) {
      throw new Error('OpenAI not initialized. Please set API key first.');
    }
    ...
    try {
      ...
      throw new Error(`LLM processing with image failed: ${error.message}`);
    }
  }
