...
const api = {
  ...
  // API Connection
  testApiConnection: (): Promise<{ success: boolean; connected?: boolean; error?: string }> => 
    ipcRenderer.invoke('test-api-connection'),

  // LLM Processing
  processWithLLM: (data: { text: string; agentId: string }): Promise<{ success: boolean; result?: unknown; error?: string }> => 
  ...
  saveAudioFile: (data: { data: number[]; mimeType: string }): Promise<{ filePath: string }> =>
